...
def findMiniMax(kalaha, max_depth, maximizing_player, selected_field = None):
    global count
    count = count + 1
    if max_depth == 0 or kalaha.game_running == False: 
        # evaluate board state
        return kalaha.fields[13] - kalaha.fields[6], selected_field
    ...
    if maximizing_player:
        ...
        for field in range(7, 13): # iterate over all possible moves to make
            if kalaha.fields[field] > 0: # check if move is valid
                ...
                # print("COPY", kalaha.fields)
                value, value_field = findMiniMax(deepcopy(kalaha),  max_depth if kalaha.player_turn == turn else max_depth - 1, True if kalaha.player_turn == kalaha.player_two else False, selected_field if selected_field != None else field) # run minimax on new game state
                if (value > max_value):
                    max_value = value
        ...
        for field in range(0, 6):
            if kalaha.fields[field] > 0:
                turn = kalaha.player_turn
                kalaha.move(kalaha.fields, field, simulation = True) # perform move
                value, value_field = findMiniMax(deepcopy(kalaha), max_depth if kalaha.player_turn == turn else max_depth - 1, True if kalaha.player_turn == kalaha.player_two else False, selected_field if selected_field != None else field)
                if (value < min_value):
                    min_value = value
                    selected_field = value_field
                    # selected_field = value_field if value_field != None else selected_field
